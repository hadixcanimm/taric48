 ...
 */
import { VOLUME_ICON, MUTE_ICON } from './styles.js';
...
export class Game2048 {
    ...
    init() {
        ...
        const globalMute = document.querySelector('#p2p-mute-toggle');
        if (globalMute) {
            globalMute.onclick = (e) => {
                e.stopPropagation();
                this.toggleMute();
            };
            globalMute.innerHTML = this.isMuted ? MUTE_ICON : VOLUME_ICON;
        }
        ...
        if (toggleBtn) toggleBtn.innerText = this.isTextMode ? 'SKIN' : '123';
    ...
    toggleMode() {
        ...
        if (btn) btn.innerText = this.isTextMode ? 'SKIN' : '123';
    ...
    toggleMute() {
        ...
        this.saveGame();
        const muteBtn = document.querySelector('#p2p-mute-toggle');
        if (muteBtn) {
            muteBtn.innerHTML = this.isMuted ? MUTE_ICON : VOLUME_ICON;
        }
        if (this.isMuted) {
            this.stopTiktak();
